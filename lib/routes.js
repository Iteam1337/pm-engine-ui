...
const engine = require('@iteam1337/engine') // TODO: peka om lokalt
...
function stream() {
  return (
    _.merge([_.values(cache), engine.cars.fork()])
      ...
      .map(car =>
        Object.assign({}, car, {
          tail: car.tail ? car.tail.filter(t => t[2] > Date.now() - 120000) : []
        })
      )
...
function register(app, io) {
  io.on('connection', function(socket) {
    ...
    stream().each(cars => socket.volatile.emit('cars', cars))
  })
}
